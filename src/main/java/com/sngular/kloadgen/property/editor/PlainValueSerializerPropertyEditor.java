...
@Slf4j
public class PlainValueSerializerPropertyEditor extends PropertyEditorSupport implements ActionListener, TestBeanPropertyEditor, ClearGui {

  private final JPanel panel = new JPanel();

  private JComboBox<String> serializerComboBox;

  public PlainValueSerializerPropertyEditor() {
    this.init();
  }

  public PlainValueSerializerPropertyEditor(final Object source) {
    super(source);
    this.init();
    this.setValue(source);
  }

  public PlainValueSerializerPropertyEditor(final PropertyDescriptor propertyDescriptor) {
  ...
  private void fillSerializer(final JComboBox<String> objectJComboBox) {
    ...
    serializerComboBox.removeAllItems();
    final Reflections reflections =
        new Reflections(new ConfigurationBuilder()
                            .addUrls(ClasspathHelper.forClass(Serializer.class))
                            .filterInputsBy(new FilterBuilder()
                                                .includePackage("org.apache.kafka.common.serialization.StringSerializer"))
