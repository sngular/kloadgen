...
public final class ConfluentSchemaRegistry implements SchemaRegistryAdapter {
  ...
  public AbstractParsedSchema<?> getSchemaBySubject(final String subjectName) {
    try {
      ...
      final var parsedSchema = this.schemaRegistryClient.getSchemaBySubjectAndId(subjectName, schemaMetadata.getId());
      return extractSchema(parsedSchema, subjectName, schemaMetadata.getSchemaType());
    } catch (RestClientException | IOException e) {
      throw new KLoadGenException(e.getMessage());
    }
  }

  public AbstractParsedSchema<?> getSchemaBySubjectAndId(final String subjectName,
    ...
    try {
      final var parsedSchema = this.schemaRegistryClient.getSchemaBySubjectAndId(subjectName, schemaMetadataAdapter.getGlobalId());
      return extractSchema(parsedSchema, subjectName, schemaMetadataAdapter.getType());
    } catch (final IOException | RestClientException e) {
      throw new KLoadGenException(e);
    }
  }

  private AbstractParsedSchema<?> extractSchema(final ParsedSchema parsedSchema, final String schemaName, final String schemaType) {
    return switch (SchemaTypeEnum.valueOf(schemaType)) {
      case AVRO -> new AvroParsedSchema(schemaName, (Schema) parsedSchema.rawSchema());
      case PROTOBUF -> new ProtobufParsedSchema(schemaName, (ProtoFileElement) parsedSchema.rawSchema());
      case JSON -> new JsonParsedSchema(schemaName, parsedSchema.rawSchema().toString());
    };
  }
